    ...
    private void OnDisable()
    {
        switch(_invaderType)
        {
            case INVADER_TYPE.FIRST_TYPE:
                _invaderManager.Invader_1.Remove(this.transform);
            break;
        ...
        _invaderManager.MaxTimerMoveInvader_1 -= .017f;
        _invaderManager.MaxTimerMoveInvader_2 -= .017f;
        _invaderManager.MaxTimerMoveInvader_3 -= .017f;

        if(_invaderManager.Invader_1.Count + _invaderManager.Invader_2.Count + _invaderManager.Invader_3.Count <= 1)
        {
            _invaderManager.MaxTimerMoveInvader_1 = 0f;
